...
const Element = React.memo(({ symbol, name, number, accent = false, gifUrl, videoUrl, thumbnail, onClick }: ElementProps) => {
  ...
  const [isVisible, setIsVisible] = useState(false); // Start false for mobile optimization
  const elementRef = useRef<HTMLDivElement>(null);
  const isComingSoon = name === 'Coming Soon';

  // MOBILE-ONLY PAGE: Always treat as mobile device
  const isMobile = true;

  // Intersection Observer for performance optimization (ALWAYS ENABLED FOR /sp)
  useEffect(() => {
    const element = elementRef.current;
    if (!element) return;

    // Aggressive mobile optimization: Only load visible GIFs
    const observer = new IntersectionObserver(
      ...
      {
        rootMargin: '100px', // Smaller margin for mobile
        threshold: 0.01
      }
    );
  ...
  const handleMouseEnter = useCallback(() => {
    // Mobile-only page: No hover effects
  }, []);
  ...
  const handleClick = useCallback(() => {
    // Mobile-only: Direct click to open modal
    onClick?.();
  }, [onClick]);

  // Mobile-only page: Never show GIF in grid (only in modal)
  const shouldShowGif = false;
  ...
  return (
    <div
      ...
      style={{
        ...
        // Mobile-only: Never use will-change to save memory
        willChange: 'auto'
      }}
    >
